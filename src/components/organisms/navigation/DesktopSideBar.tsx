        ...
        <nav class="space-y-2 px-2">
          {MAIN_NAVIGATION.map((navItem) => (
            <Link
              key={navItem.path}
              href={navItem.path}
              activeClassName="!text-emerald-700 !hover:bg-transparent"
              class="group flex items-center rounded-md px-4 py-2 text-base font-medium text-white transition-colors hover:bg-emerald-700/40">
              {navItem.Icon && (
        ...
        <nav class="space-y-1 px-2">
          {SECONDARY_NAVIGATION.map((navItem) => (
            <Link
              key={navItem.path}
              href={navItem.path}
              class="group flex items-center rounded-md px-2 py-2 text-base font-medium text-white hover:bg-emerald-800">
              {navItem.Icon && (
                <navItem.Icon class="mr-3 h-6 w-6 flex-shrink-0 text-emerald-300" />
              )}
              {navItem.name}
